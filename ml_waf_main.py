...
def create_balanced_training_data() -> pd.DataFrame:
    ...
    if df.empty or "is_malicious" not in df.columns:
        # nothing to load â€” return synthetic benign dataset so training code can continue
    ...
    # Oversample malicious if required â€” user might plug custom generator here
...
class WAFAPIServer:
                ...
                try:
                    ...
                    if USE_SIGNATURES and any([
                        ...
                        feats.get('has_portscan_signature', 0),
                            # Add new flags here:
                        feats.get('has_fuzzing_signature', 0),
                        feats.get('has_ftp_exploit_signature', 0),
                        feats.get('has_icmp_flood_signature', 0),
                        feats.get('has_ssh_ftp_bruteforce', 0),
                    ]):
    ...
    def _setup_dashboard_routes(self):
        ...
        def api_attack_types_chart():
            ...
            icmp_flood_count = sum(
                1 for r in getattr(proxy, "request_log", [])
               if r.get("is_malicious") and isinstance(r.get("features"), dict) and r["features"].get("icmp_flood_count", 0) > 0
           )
            fuzzing_count = sum(
               1 for r in getattr(proxy, "request_log", [])
              if r.get("is_malicious") and isinstance(r.get("features"), dict) and r["features"].get("fuzzing_signature_count", 0) > 0
          )
            ddos_count = sum(
             1 for r in getattr(proxy, "request_log", [])
             if r.get("is_malicious") and isinstance(r.get("features"), dict) and r["features"].get("ddos_count", 0) > 0
      )
            brute_force_count = sum(
             1 for r in getattr(proxy, "request_log", [])
             if r.get("is_malicious") and isinstance(r.get("features"), dict) and r["features"].get("brute_force_count", 0) > 0
      )
            port_scanning_count = sum(
              1 for r in getattr(proxy, "request_log", [])
             if r.get("is_malicious") and isinstance(r.get("features"), dict) and r["features"].get("port_scanning_count", 0) > 0
       )
            ftp_exploiting_count = sum(
             1 for r in getattr(proxy, "request_log", [])
             if r.get("is_malicious") and isinstance(r.get("features"), dict) and r["features"].get("ftp_exploiting_count", 0) > 0
     )
            ...
            return jsonify({
             ...
             "icmp_flood": icmp_flood_count,
              "fuzzing": fuzzing_count,
              "ddos": ddos_count,
             "brute_force": brute_force_count,
             "port_scanning": port_scanning_count,
              "ftp_exploiting": ftp_exploiting_count
})


        @self.app.route("/api/threat_severity_chart")
        def api_threat_severity_chart():
            # placeholder static values â€” replace with real aggregation if you compute severities
        ...
        def api_recent_threats():
          raw = getattr(proxy, "request_log", [])
          formatted = []
          for r in raw:
             if not r.get("is_malicious"):
                continue
             rec = r.copy()
                # If 'timestamp' missing but 'ts' exists, convert it
             if "timestamp" not in rec and "ts" in rec:
                    rec["timestamp"] = datetime.fromtimestamp(rec["ts"]).isoformat()
             formatted.append(rec)
            # Return the last 10 malicious entries, most recent last
